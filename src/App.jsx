...
function App() {
  ...
  const getTimeLeft = (timer) => {
    const now = Date.now();
    if (!timer.isRunning) {
      const pausedAt = timer.pausedAt || now;
      return Math.max(0, Math.floor((timer.targetTimestamp - pausedAt) / 1000));
    }
    return Math.max(0, Math.floor((timer.targetTimestamp - now) / 1000));
  };
  ...
  const toggleTimer = (id) => {
    setTimers(prev => prev.map(t => {
      ...
      const now = Date.now();
      if (t.isRunning) {
        return { ...t, isRunning: false, pausedAt: now };
      } else {
        const pauseTime = t.pausedAt ? (now - t.pausedAt) : 0;
        return { 
          ...t, 
          isRunning: true, 
          targetTimestamp: t.targetTimestamp + pauseTime,
          pausedAt: null 
        };
      }
    }));
  ...
  return (
    <div className="min-h-screen w-full bg-zinc-950 p-8 sm:p-14 max-w-7xl mx-auto">
      <header className="mb-16 fade-in-up">
        ...
        <p className="text-xs font-bold uppercase tracking-widest text-zinc-600 mt-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {timers.map((timer) => (
          <div key={timer.id} className="fade-in-up">
            <TimerCard 
              timer={{ ...timer, remainingSeconds: getTimeLeft(timer) }} 
        ...
        {timers.length === 0 && (
          <div className="col-span-full py-32 flex flex-col items-center justify-center border-2 border-dashed border-zinc-900 rounded-3xl opacity-50">
            <Plus className="w-10 h-10 text-zinc-700 mb-6" />
            <Button 
              variant="outline"
              onClick={() => { setEditingTimer(null); setIsModalOpen(true); }}
